...
- integrating a service displayed on the SAP API Business Hub
...
Please note that the inclusion of a service displayed on the SAP API Business Hub originates from a S/4 HANA Cloud system. While large parts of the tutorial are directed fpr the clients to consume the service from a real S/4 HANA Cloud system, the service will eventually not be consumed from a real system but from a local copy on SAP Cloud Platform. This is because within this tutorial and using a SAP Cloud Platform trial account no real S/4 HANA Cloud system can be provided and configured.
